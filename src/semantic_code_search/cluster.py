...
clustering_functions = {
    "KDTrees": clustering_kdtrees,
    "torch.topk": _get_clusters,
        "suffix-trees": get_suffix_tree_clusters,
        "inverted-index":inverted_index_clustering
...
def do_cluster(args, model):
    ...
    with gzip.open(args.path_to_repo + '/' + '.embeddings', 'r') as f:
        ...
        clustering_function = clustering_functions.get(args.cluster_data_structure)
        # start = time.time()
        clusters = clustering_function(dataset, args.cluster_max_distance)
        # end = time.time()
        # print("Improved Time taken: ", end-start)
        filtered_clusters = []
        for c in (clusters):
