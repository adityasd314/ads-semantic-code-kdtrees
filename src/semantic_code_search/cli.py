...
def main():
    ...
    parser.add_argument('--cluster-data-structure', choices=['KDTrees', 'torch.topk', 'suffix-trees', 'inverted-index'], default='KDTrees', required=False,
                        help='Data structure to use for clustering. KDTrees is the fastest, but requires more memory. torch.topk is slower but uses less memory. Suffix trees are the slowest but use the least memory. Inverted index used for token based clustering')
    ...
    print("Give them hell for that is all they deserve !")
    if args.embed:
        embed_func(args)
    elif args.cluster:
        cluster_func(args)
